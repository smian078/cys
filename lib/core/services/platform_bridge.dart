...
class AndroidBridge {
  ...
  AndroidBridge() {
    _channel.setMethodCallHandler((call) async {
      if (call.method == 'shareIntent') {
        _sharedController.add(SharedPayload.fromMap(Map<String, dynamic>.from(call.arguments as Map)));
      }
    });
  }
  ...
  Future<bool> openApp(String package) async => (await _channel.invokeMethod<bool>('openApp', {'package': package})) ?? false;
  Future<bool> openUrl(String url) async => (await _channel.invokeMethod<bool>('openUrl', {'url': url})) ?? false;
  Future<void> shareText(String text) => _channel.invokeMethod('shareText', {'text': text});
  Future<bool> shareFile(String path) async => (await _channel.invokeMethod<bool>('shareFile', {'path': path})) ?? false;
  void dispose() => _sharedController.close();
}

class SharedPayload {
  const SharedPayload({required this.text, required this.paths, required this.mime});
  final String text;
  final List<String> paths;
  final String mime;
  factory SharedPayload.fromMap(Map<String, dynamic> map) => SharedPayload(text: (map['text'] as String?) ?? '', paths: ((map['paths'] as List?) ?? const []).cast<String>(), mime: (map['mime'] as String?) ?? '');
}
