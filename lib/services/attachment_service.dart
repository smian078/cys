...
class AttachmentService {
  ...
  Future<ChatAttachment> persist(
    ...
    return ChatAttachment(
      id: id,
      path: target.path,
      name: displayName ?? p.basename(source.path),
      mimeType: mime,
      size: size,
      sourceUrl: sourceUrl,
  ...
  Future<ChatAttachment> persistBytes(
    List<int> bytes, {
  ...
  Future<AttachmentContext> buildContext(ChatAttachment attachment) async {
    ...
    if (attachment.mimeType == 'application/pdf' ||
        p.extension(attachment.path).toLowerCase() == '.pdf') {
      final document = PdfDocument(inputBytes: bytes);
      final extracted = PdfTextExtractor(document).extractText();
      document.dispose();
      return AttachmentContext(
        attachment: attachment,
        extractedText: extracted.length > 120000
            ? extracted.substring(0, 120000)
            : extracted,
      );
    }
    if (_isTextLike(attachment.mimeType, attachment.path)) {
      ...
      return AttachmentContext(
        attachment: attachment,
        extractedText: text.length > 120000 ? text.substring(0, 120000) : text,
      );
    }
    return AttachmentContext(
      attachment: attachment,
