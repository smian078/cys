...
class PipelineCoordinator {
  ...
  Stream<CystemStreamEvent> run(
    ...
    final route = _router.route(
      ...
      selectedModel: request.settings.model,
    );
    final brainModel = request.settings.model;
    var research = '';
    var sources = <SourceRecord>[];
    if ((route.useWeb || route.useImages) &&
        geminiKey != null &&
        geminiKey.isNotEmpty) {
      if (route.useWeb) {
        try {
          final result = await _gemini.research(
            apiKey: geminiKey,
        ...
        try {
          final images = await _gemini.imageSearch(
            apiKey: geminiKey,
            query: request.message,
          );
          research =
          ...
          if (_attachments != null) {
            for (var i = 0; i < images.length; i++) {
              ...
              research +=
                  'Image: ${image.title}${image.sourceUrl == null ? '' : '\nSource: ${image.sourceUrl}'}\n';
            }
          }
        } catch (e) {
    ...
    if (nvidiaKey == null || nvidiaKey.isEmpty) {
      yield const StreamError(
        'Add an NVIDIA API key in Settings → Providers to use the main brain.',
      );
      return;
    }
    ...
    for (var turn = 0; turn < 6; turn++) {
      ...
      await for (final event in _nim.stream(
        ...
        if (event is ToolCallDelta) {
          final acc = pendingTools.putIfAbsent(
            event.index,
            () => _ToolAccumulator(),
          );
    ...
    yield const StreamError(
      'Tool execution exceeded the maximum continuation depth.',
    );
  }

  Future<String> _analyzeAttachment({
    required AttachmentContext context,
    ...
    final content = <Map<String, dynamic>>[
      {
        'type': 'text',
        'text':
            'Analyze this attachment for factual context only. Extract visible text, objects, people, UI elements, charts, colors, spatial relationships, and important details. Do not answer the end user directly. Return concise factual notes.',
      },
    ];
    if (context.imageDataUri != null) {
      content.add({
        'type': 'image_url',
        'image_url': {'url': context.imageDataUri},
      });
    ...
    await for (final event in _nim.stream(
      ...
      apiKey: apiKey,
      model: ModelIds.nemotronNanoOmni,
      messages: [
